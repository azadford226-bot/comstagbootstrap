...
import {
  ...
  getOAuthLoginBaseUrl,
  OAUTH_REGISTRATION_IDS,
  type OAuthProviderInfo,
} from "@/lib/api/oauth-providers";
import {
  setAccessToken,
  setRefreshToken,
  setUserEmail,
...
export default function LoginForm() {
  ...
  const [oauthProviders, setOauthProviders] = useState<OAuthProviderInfo[] | undefined>(
    undefined
  );

  const oauthBase = useMemo(() => getOAuthLoginBaseUrl(), []);

  useEffect(() => {
    fetchOAuth2Providers().then((data) => {
  ...
  return (
    <div className="max-w-[600px] mx-auto">
      ...
      >
        ...
        {/* SSO — providers from API; live redirect only when backend URL + registered IdP */}
        {!isCodeSent && oauthProviders !== undefined && (
          <>
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-white text-text-body">Or continue with</span>
              </div>
            </div>
            <p className="text-xs text-gray-500 mb-3">
              Single sign-on uses your Spring Boot app at the same origin as your API. Set{" "}
              <code className="text-[11px] bg-gray-100 px-1 rounded">NEXT_PUBLIC_OAUTH_LOGIN_BASE_URL</code>{" "}
              when the frontend is hosted separately (e.g. Vercel → Railway).
            </p>
            {oauthProviders.length === 0 ? (
              <p className="text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
                Could not load SSO providers. Check API connectivity and try again.
              </p>
            ) : (
                ...
                {oauthProviders.map((provider) => {
                  ...
                  const icon =
                    id === "google" ? (
                      ...
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="#181717" aria-hidden>
                  ...
                  const canRedirect =
                    !!oauthBase &&
                    provider.enabled &&
                    OAUTH_REGISTRATION_IDS.has(id);
                  const href =
                    canRedirect && oauthBase
                      ? `${oauthBase}${getOAuthAuthorizationPath(id)}`
                      : undefined;
                  return canRedirect && href ? (
                    <a
                      key={id}
                      href={href}
                      className="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-medium text-gray-800 bg-white hover:bg-gray-50 transition-colors"
                    >
                      {icon}
                      {provider.name}
                    </a>
                    ...
                    <button
                      ...
                      title={
                        !oauthBase
                          ? "Set NEXT_PUBLIC_API_BASE_URL or NEXT_PUBLIC_OAUTH_LOGIN_BASE_URL"
                          : !provider.enabled
                          ? "Provider not configured on the server yet"
                          : "Provider not registered in this deployment"
                      }
                    >
                      <span className="opacity-50">{icon}</span>
                      {provider.name}
                      <span className="absolute -top-2 -right-2 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded-full">
                        Soon
                      </span>
                    </button>
                  );
