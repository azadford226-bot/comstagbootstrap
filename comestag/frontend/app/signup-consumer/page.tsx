...
export default function ConsumerSignupPage() {
  ...
  const handleInterestsChange = (selectedIds: number[]) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const validateStep1 = (): boolean => {
    ...
    // Check industryId separately
    if (!formData.industryId || formData.industryId === 0) {
      setError("Industry is required");
      return false;
    }

    // Check interests separately
    ...
    // Validate website URL starts with https://
    if (
      formData.website &&
      !formData.website.toLowerCase().startsWith("https://")
    ) {
