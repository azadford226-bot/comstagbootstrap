...
export default function PublicProfilePage() {
  ...
  const loadPublicProfile = async () => {
    try {
      ...
      // Fetch profile first to determine type
      ...
      if (profileRes.success && profileRes.data) {
        ...
        if (isOrganizationProfile(profileRes.data)) {
          ...
          // Load consumer-specific data (testimonials they wrote)
          const testimonialsRes = await getTestimonials();

          if (testimonialsRes.success && testimonialsRes.data) {
            const allTestimonials = testimonialsRes.data.items || [];
            const consumerTestimonials = allTestimonials.filter(
              (t) => t.consumerId === userId
            );
            setTestimonials(consumerTestimonials);
          }
        }
      }
    } catch (error) {
